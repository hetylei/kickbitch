...
Validator.messageSource['zh-com'] = [
...
ValidationUtils = {
	...
	isDate : function(v,dateFormat) {
		...
		var MIN = "mm";
		var SEC = "ss";
		var regex = '^'+dateFormat.replace(YEAR,'\\d{4}').replace(MONTH,'\\d{2}').replace(DAY,'\\d{2}').replace(HOUR,'\\d{2}').replace(MIN,'\\d{2}').replace(SEC,'\\d{2}')+'$';
		...
		var hour = v.substr(dateFormat.indexOf(HOUR),2);if (hour=='') hour = '00';
		var min = v.substr(dateFormat.indexOf(MIN),2);if (min=='') min = '00';
		var sec = v.substr(dateFormat.indexOf(SEC),2);if (sec=='') sec = '00';
		
		var d = new Date(ValidationUtils.format('%s/%s/%s %s:%s:%s',[year,month,day,hour,min,sec]));
		return ( parseInt(month, 10) == (1+d.getMonth()) ) && 
					(parseInt(day, 10) == d.getDate()) && 
					(parseInt(year, 10) == d.getFullYear() ) &&
					(parseInt(hour, 10) == d.getHours() ) &&
					(parseInt(min, 10) == d.getMinutes() ) &&
					(parseInt(sec, 10) == d.getSeconds() ) ;		
	},
	...
	fireSubmit: function(form) {
 	...
 	getMessageSource : function() {
 		...
 		var messageSource = Validator.messageSource['zh-com'];
...
Validation.addAllThese([
	...
	['validate-date', function(v,elm,args,metadata) {
			...
			metadata._error = ValidationUtils.format(ValidationUtils.getI18nMsg(metadata.className),[dateFormat,dateFormat.replace('yyyy','2006').replace('MM','03').replace('dd','12').replace('HH','08').replace('mm','10').replace('ss','15')]);
